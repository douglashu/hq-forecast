...
import com.hq.scrati.framework.invoker.HttpsPostInvoker;
@Component("httpsAdapter")
public class HttpsAdapter implements IAdapter {
	...
	@Resource
	private HttpsPostInvoker httpsPostInvoker;
	...
	public Object excute(TesbService tesbService, Map<String, Object> parameters) throws Exception {
		...
		if("1".equals(tesbHttp.getUrlStyle())){
			if("post".equals(tesbHttp.getMethod().toLowerCase())){
				return httpsPostInvoker.invokeMethod(tesbHttp, httpParameters);
			}
			
		}else{
//			return restFulInvoker.invokeMethod(tesbHttp, httpParameters);
